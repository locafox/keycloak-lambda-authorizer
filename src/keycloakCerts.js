...
function KeycloakPublicKeyFetcher(url, realm) {
    let certsUrl = realm ? `${url}/auth/realms/${realm}/protocol/openid-connect/certs` : url;
    if (url && url.indexOf('/auth') > 0) {
        certsUrl = realm ? `${url}/realms/${realm}/protocol/openid-connect/certs` : url;
    }
    return {
